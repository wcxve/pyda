...
from pyda.utils import tehist_gecam, thist_gecam, ehist_gecam
from pyda.utils import met_to_utc, utc_to_met
...
def plot_ehist(evt_file, t0, trange, emin=8.0, emax=8000.0):
    ...
    for i in range(len(dets)):
        ...
        hg = ehist_gecam(evt_file, det, 0, trange, erange, met0)
        ...
        lg = ehist_gecam(evt_file, det, 1, trange, erange, met0)
...
def plot_thist(evt_file, t0, tstart, tstop, dt, emin=8.0, emax=8000.0):
    trange = [tstart, tstop]
    erange = [emin, emax]
    sat = _get_sat_name(evt_file)
    met0, utc0 = _get_met_and_utc(t0, sat)
    ...
    rates_hg = []
    for det in DET[sat]:
        lc = thist_gecam(evt_file, det, 0, trange, erange, dt, t0)
        rate = lc['counts'] / lc['exposure']
        rates_hg.append(np.append(rate, rate[-1]))

    rates_lg = []
    for det in DET[sat]:
        lc = thist_gecam(evt_file, det, 1, trange, erange, dt, t0)
        rate = lc['counts'] / lc['exposure']
        rates_lg.append(np.append(rate, rate[-1]))

    n_hg = int(np.floor(np.log10(np.max(rates_hg))))
    n_lg = int(np.floor(np.log10(np.max(rates_lg))))
...
def plot_tehist(evt_file, t0, tstart, tstop, dt, emin=8.0, emax=8000.0):
    trange = [tstart, tstop]
    erange = [emin, emax]
    sat = _get_sat_name(evt_file)
    met0, utc0 = _get_met_and_utc(t0, sat)
    ...
    ebins_hg = []
    for det in DET[sat]:
        hist = tehist_gecam(evt_file, det, 0, trange, erange, dt, t0)
        ...
        ebins = np.append(hist['ebins'].sel(edge='left')[i1:i2],
                          hist['ebins'].sel(edge='right')[i1:i2][-1])
    ...
    for det in DET[sat]:
        if sat == 'GECAM-C' and det in [6, 12]:
            ...
            ebins_lg.append([])
            continue
        hist = tehist_gecam(evt_file, det, 1, trange, erange, dt, t0)
        ebins_width = np.squeeze(hist['ebins'].diff('edge'))
        rate = hist['counts'] / hist['exposure'] / ebins_width
        indices = np.flatnonzero(np.any(rate > 0.0, axis=1))
        i1 = indices.min()
