...
def wstat_xspec(mod_rates, obs_counts, back_counts, obs_exp, back_exp):
    stat = 0.0
...
def wstat(s, n_on, n_off, t_on, t_off):
    n = len(n_on)
    stat = 0.0
    B = np.empty(n)
    d1 = np.empty(n)
    ...
    for i in range(n):
        ...
        if on == 0.0:
            stat += Si + off*log(v1)
            B[i] = off / v1 if off > 0.0 else 0.0
            d1[i] = 1.0
            d2[i] = 0.0
        else:
            if off == 0.0:
                if Si <= on / v2:
                    ...
                    v7 = on/Si
                    stat += Si + on*(log(v7) - 1.0)
                    B[i] = 0.0
                    d1[i] = 1.0 - v7
                    d2[i] = v7/Si
            else:
                c = a * (on + off) - v1 * Si
                d = sqrt(c*c + v4 * off * Si)
                b = (c + d) / v3
                stat += Si + v1 * b \
                        - on * (log((Si + a*b)/on) + 1) \
                        - off * (log(b/off) + 1)
                ...
                v7 = on + off
                d1[i] = -(c - d + 2*Si) / (v5*Si)
                d2[i] = (v7 + (v1*(on-off)*Si - a*v7*v7)/d) / (2*Si*Si)

    return stat, B/t_off, t_on*d1, t_on*t_on*d2
...
def wstat2(rate_src, n_on, n_off, t_on, t_off):
    n = len(n_on)
    stat = 0.0
    mu_bkg = np.empty(n)
    d1 = np.empty(n)
...
if __name__ == '__main__':
    ...
    t_on = 1.0
    t_off = 1.0
    print(wstat_xspec(s, n_on, n_off, t_on, t_off))
    print(wstat(s, n_on, n_off, t_on, t_off))
    print(wstat2(s, n_on, n_off, t_on, t_off))